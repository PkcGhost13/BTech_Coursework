...
int main(void)
{
    ...
    int twoComplement_n[100];
    int carry=1;
    for(int i=k-1;i>=0;i--)
    {
        if(oneComplement_n[i]==1&&carry==1)
        {
            twoComplement_n[i]=0;
        }
        else if(oneComplement_n[i]==0 && carry==1)
        {
            twoComplement_n[i]=1;
            carry=0;
        }
        ...
        {
            twoComplement_n[i]=oneComplement_n[i];
        }

    }

    cout<<"The two's complement is : ";
    for(int i=0;i<k;i++)
    {
        cout<<twoComplement_n[i];
    }
    cout<<endl;
}
